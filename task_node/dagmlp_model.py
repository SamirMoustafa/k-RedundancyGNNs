from torch import Tensor, cat, mean, ones, stack, sum, zeros_like
from torch.nn import Dropout, Linear, Module, ModuleDict, Parameter, ReLU, Sequential
...
class DAGMLPLayer(MessagePassing):
    def __init__(self, nn, eps=-1.0, train_eps=False):
        super().__init__()
        self.nn = nn
...
class DAGMLP(Module):
    def __init__(self, dim_features, dim_target, **config):
        ...
        if config["combine_function"] == "sum":
            self.combine_function = lambda x: sum(stack(x), dim=0)
        elif config["combine_function"] == "mean":
            self.combine_function = lambda x: mean(stack(x), dim=0)
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            ...
            x = self.dag_layers[f"dag_mlp_{i}"](x, feature, edge_index_i, dag_edge_attr_i)

        if self.combine_multi_height:
            x = [x[readout_i] for i, readout_i in enumerate(dag_readouts)]
            x = self.combine_function(x)
        else:
            x = x[dag_readouts[-1]]
        x = self.dropout(x)
        x = self.last_layer(x)
